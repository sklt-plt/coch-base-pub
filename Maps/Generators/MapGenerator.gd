...
export (Dictionary) var int_objects = {
	...
	"locked_doors" : [{"path" : "/Ent/Interactable/LockedDoor/LockedDoor4x4.tscn"},		# needs to be sorted from tallest to lowest
...
export (int) var debug_exit_direction = -1
...
func prepare():
	...
	for door in int_objects["locked_doors"]:
	...
	if not _walls_generator:
		_walls_generator = RoomWallsGenerator.new(_owner, _wall_materials,
			...
			int_objects["locked_doors"], debug_exit_direction)
	...
	var oldAG = get_parent().find_node("arena_interior_generator*") # ba chuj to ??!!
	if oldAG:
		oldAG.queue_free()
		_arena_generator = null

	if not _arena_generator: # na chuj to
	...
	var oldLDG = get_parent().find_node("link_door_generator*")
	if oldLDG:
		oldLDG.queue_free()
		_door_generator = null
	...
	if not _door_generator:
		...
		_door_generator.set_owner(_owner)
...
func generate_pass_two(var root):
	...
	for node in nodes:
		...
		if node is GeneratedRoom:
			...
			_door_generator.make_doors(node, link_door_material)

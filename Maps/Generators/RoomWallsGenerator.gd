...
func _init(var owner, var wall_materials, var floor_materials,
	...
	var locked_doors, var debug_exit):
		_owner = owner
...
func find_exits_origin_random(var room_geometry: RoomGeometry, var wall_id: int, var link_size: Vector2):
	...
	if room_geometry.size.x < 4 or room_geometry.size.z < 4:
		print("Warning: Room smaller than 4x4 in one direction. Exits may get player stuck")

	match wall_id:
		WALL_ID_NORTH:
			x_offset = _rng.randi_range(1, room_geometry.size.x-1-link_size.x)
			z_offset = 0
		WALL_ID_SOUTH:
			x_offset = _rng.randi_range(1, room_geometry.size.x-1-link_size.x)
		...
		WALL_ID_WEST:
			x_offset = 0
			z_offset = _rng.randi_range(1, room_geometry.size.z-1-link_size.x)
		WALL_ID_EAST:
...
func make_holed_wall(var room_geometry, var named, var make_corridor):
	...
	# override if adjecent room has highier walls
	...
	if make_corridor:
		...
		# ceiling
		# is there locked door
		var child_geometry = get_node_or_null(desired_link["target"])
		var child_data
		if child_geometry:
			child_data = get_node(child_geometry.tree_ref)

#		if ((child_data and child_data.traits.has(GeneratedRoom.ROOM_TRAITS.LOCKED_DOOR)) or
#			_make_ceilings):
			# TODO wtf is going on with doors?
		if _make_ceilings:
	...
	if from_geometry and to_geometry:
		...
		if not _make_ceilings:
			...
			if from_geometry.size.y < to_geometry.size.y:
				max_y = to_geometry.size.y
			else:
				max_y = from_geometry.size.y
		else:
			if from_geometry.size.y < to_geometry.size.y:
				max_y = from_geometry.size.y
			else:
		...
		if wall_id == WALL_ID_NORTH or wall_id == WALL_ID_SOUTH:
			if from_geometry.size.x > to_geometry.size.x:
				max_x = to_geometry.size.x-2
			else:
				max_x = from_geometry.size.x-2
		else:
			if from_geometry.size.z > to_geometry.size.z:
				max_x = to_geometry.size.z-2
			else:
				max_x = from_geometry.size.z-2

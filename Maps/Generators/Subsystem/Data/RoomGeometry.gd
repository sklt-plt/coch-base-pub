...
func move_to_offset(): # called when player enters neighbour room
...
func open_doors_except_to(var room):
	if children.empty():
		children = get_children()

	var links = used_exits()

	# find desired direction
	var dir
	for l in links:
		if l["target"] == room.get_path():
			dir = l["direction"]
	...
	for c in children:
		...
		if c is LinkDoor and c.direction_ == dir:
			c.open()

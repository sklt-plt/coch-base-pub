...
export (int) var _num_of_random_paths = 15					# how many random walkt to take in corridor rooms
export (PATH_WIDTHS_OTHERS) var side_path_widths = 0	# which distribution of random paths to use
...
func reset(var tiles: String,
	var rng: RandomNumberGenerator):
		_tiles = load(Globals.content_pack_path + tiles)
		_other_paths_width_table = PATH_WIDTHS_TABLES[side_path_widths]
		_rng = rng
...
func make_rectangular_path_piece(var point_a: Vector3, var point_b: Vector3, var gmap: GridMap, var reserved_path_tile : int,
	...
	var real_corners = find_real_corners(point_a, point_b, thickness_left, thickness_right, overdraw)

	var desired_wall_height = wall_height
	if clear_awnings:
		desired_wall_height = _current_bounds.y+1

	draw_reserved_path(real_corners["x1"], real_corners["x2"], real_corners["z1"], real_corners["z2"], desired_wall_height, reserved_path_tile, gmap)

	draw_walls(real_corners["x1"], real_corners["x2"], real_corners["z1"], real_corners["z2"], wall_height, gmap)

func find_real_corners(var point_a, var point_b, var thickness_left, var thickness_right, var overdraw):
	var x1 = point_a.x
	var x2 = point_b.x
	...
	return {"x1" : real_x1, "x2" : real_x2, "z1" : real_z1, "z2" : real_z2}

func draw_reserved_path(var real_x1, var real_x2, var real_z1, var real_z2, var desired_wall_height, var reserved_path_tile, var gmap):
	for x in range(real_x1, real_x2+1):
		for z in range(real_z1, real_z2+1):
			if (is_floor_tile_in_bounds(Vector3(x, 0, z))
				...
				for h in range(1, desired_wall_height):
